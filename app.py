...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Performance: Convert to list of dicts for faster access
        rows1 = df1.to_dict('records')
        rows2 = df2.to_dict('records')

        # Pre-normalize values
        norm1_list = [normalize_arabic(str(r[col1])) for r in rows1]
        norm2_list = [normalize_arabic(str(r[col2])) for r in rows2]

        # Map normalized values to row indices for O(1) exact matching
        norm2_map = {}
        ...
        for idx1, n1 in enumerate(norm1_list):
            ...
            for n2 in unique_norm2:
                ...
                # Use a cache for fuzz.ratio to avoid repeated expensive calculations
