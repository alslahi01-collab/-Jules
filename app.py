...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        ...
        # Group rows in File 2 by their normalized value
        norm2_map = {}
        ...
        for idx1, row1 in enumerate(records1):
            ...
            if norm1 in norm2_map:
                ...
                # Important: we don't 'continue' here because the original logic also checks
                # for fuzzy matches even if an exact match is found (it was O(N*M) exhaustive).
                # Actually original logic DID 'continue' after finding norm1 == norm2 for a specific row pair.
                # But it would still check other row pairs.
                # Our optimized logic iterates through unique_norms2.
                # If we match norm1 == norm2, we append and move to next norm2 in unique_norms2.

            for norm2 in unique_norms2:
                if norm1 == norm2:
                    continue # Already handled in exact match O(1) block above
                
                # Check results cache for symmetric similarity
