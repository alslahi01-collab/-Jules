...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Optimization: Pre-convert to records and use unique value grouping
        records1 = df1.to_dict('records')
        records2 = df2.to_dict('records')

        norm_vals1 = [normalize_arabic(str(r.get(col1, ""))) for r in records1]
        norm_vals2 = [normalize_arabic(str(r.get(col2, ""))) for r in records2]

        # Group indices by normalized value for O(1) lookup and fewer fuzzy calls
        norm_map2 = {}
        for idx2, nv2 in enumerate(norm_vals2):
            if not nv2 or nv2 == 'nan': continue
            if nv2 not in norm_map2:
                norm_map2[nv2] = []
            norm_map2[nv2].append(idx2)
            
        fuzz_results_cache = {}

        for idx1, nv1 in enumerate(norm_vals1):
            if not nv1 or nv1 == 'nan': continue

            for nv2, indices2 in norm_map2.items():
                if nv1 == nv2:
                ...
                # Perform fuzzy matching only once per unique value pair
                pair = tuple(sorted((nv1, nv2)))
                if pair in fuzz_results_cache:
                    score = fuzz_results_cache[pair]
                else:
                    score = fuzz.ratio(nv1, nv2)
