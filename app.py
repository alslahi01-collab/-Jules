...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    """
    Processes two sheets and identifies matches based on exact and fuzzy similarity.
    Optimized to group by unique values and cache fuzzy scores.
    """
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Group indices by normalized values to avoid redundant computations
        norm_map1 = {}
        ...
        # Local cache for fuzzy results within this sheet comparison
        fuzz_cache = {}
        ...
        for n1 in unique_norms1:
            for n2 in unique_norms2:
                # Exact match
                if n1 == n2:
                    for i1 in norm_map1[n1]:
                        row1_dict = df1.iloc[i1].to_dict()
