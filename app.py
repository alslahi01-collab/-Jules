...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Pre-process df2: normalize and group indices by normalized value
        norm2_map = {}  # norm_val -> list of original indices
        ...
        for idx1, row1 in df1.iterrows():
            ...
            row1_dict = None # Lazy conversion to dict
            ...
            # 2. Fuzzy matches
            for u2 in unique_norm2:
                if norm1 == u2:
                    continue
                
                # Symmetric cache for fuzz.ratio
                pair = tuple(sorted((norm1, u2)))
                if pair in fuzz_cache:
                    score = fuzz_cache[pair]
                else:
                    score = fuzz.ratio(norm1, u2)
