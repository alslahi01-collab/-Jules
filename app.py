...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Optimization: Pre-convert DataFrame to dictionary for faster row access
        df1_dict = df1.to_dict('index')

        # Optimization: Group row indices by unique normalized value to avoid redundant matching
        norm_map1 = {}
        for idx, row in df1_dict.items():
            val = str(row[col1])
            norm = normalize_arabic(val)
            if not norm or norm == 'nan': continue
            if norm not in norm_map1:
                norm_map1[norm] = []
            norm_map1[norm].append(idx)
        ...
        for idx, row in df2.iterrows():
            ...
            norm = normalize_arabic(val)
            if not norm or norm == 'nan': continue
            if norm not in norm_map2:
                norm_map2[norm] = []
            norm_map2[norm].append(idx)

        unique_norms1 = list(norm_map1.keys())
        unique_norms2 = list(norm_map2.keys())

        # Optimization: Cache fuzzy results for unique normalized string pairs
        fuzz_results_cache = {}
            ...
            for n2 in unique_norms2:
                ...
                if n1 == n2:
                    ...
                    for idx1 in indices1:
                        # Use a copy to avoid mutating the cached row dictionary
                        row1_processed = df1_dict[idx1].copy()
                        for idx2 in indices2:
                            match_row = row1_processed.copy()
                    ...
                    # Fuzzy Match
                    # Use a sorted tuple to ensure symmetry in the cache key
