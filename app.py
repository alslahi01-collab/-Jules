...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        ...
        # Group indices by normalized value to reduce O(N*M) comparison to O(U1*U2)
        norm_map1 = {}
        for idx1, row in enumerate(records1):
            norm1 = normalize_arabic(str(row[col1]))
            if not norm1 or norm1 == 'nan': continue
            norm_map1.setdefault(norm1, []).append(idx1)

        norm_map2 = {}
        for idx2, row in enumerate(records2):
            norm2 = normalize_arabic(str(row[col2]))
            if not norm2 or norm2 == 'nan': continue
            norm_map2.setdefault(norm2, []).append(idx2)

        # Cache results of fuzzy matching between unique normalized values
        fuzz_results_cache = {}
        ...
        for norm1, indices1 in norm_map1.items():
            ...
            for norm2, indices2 in norm_map2.items():
                if norm1 == norm2: continue # Handled by exact match
                
                # Check cache for this pair of unique strings
