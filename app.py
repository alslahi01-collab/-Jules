...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Optimized processing: group rows by unique normalized values
        ...
        for idx, row in enumerate(records1):
            ...
            group1.setdefault(norm, []).append(idx)

        group2 = {}
        for idx, row in enumerate(records2):
            norm = normalize_arabic(str(row[col2]))
            if not norm or norm == 'nan': continue
            group2.setdefault(norm, []).append(idx)

        unique_norms1 = list(group1.keys())
        unique_norms2 = list(group2.keys())
        fuzzy_cache = {}

        for norm1 in unique_norms1:
            indices1 = group1[norm1]

            # 1. Exact matches
            if norm1 in group2:
                indices2 = group2[norm1]
