...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Pre-convert to records and pre-normalize to avoid redundant operations in loops
        recs1 = df1.to_dict('records')
        recs2 = df2.to_dict('records')

        norms1 = [normalize_arabic(str(r[col1])) for r in recs1]
        norms2 = [normalize_arabic(str(r[col2])) for r in recs2]

        # Group indices by normalized value to handle duplicates efficiently
        norm_map1 = {}
        for idx1, n1 in enumerate(norms1):
            if not n1 or n1 == 'nan': continue
            norm_map1.setdefault(n1, []).append(idx1)

        norm_map2 = {}
        for idx2, n2 in enumerate(norms2):
        ...
        for norm1, indices1 in norm_map1.items():
            ...
            # 2. Fuzzy matches (Optimized loop)
            for n2, indices2 in norm_map2.items():
                if norm1 == n2: continue # Already handled in exact match
                
                # Use a tuple to cache results for pairs
                pair = tuple(sorted((norm1, n2)))
                if pair not in fuzz_cache:
                    fuzz_cache[pair] = fuzz.ratio(norm1, n2)
