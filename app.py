...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    # Optimization: Pre-calculate normalized values and group row indices
    # Convert to records for much faster iteration than iterrows()
    ...
    # Results cache to avoid redundant fuzzy matching for identical strings
    score_cache = {}
    ...
    for norm1, indices1 in groups1.items():
        for norm2, indices2 in groups2.items():
            ...
            # Fuzzy matches with symmetric caching
            pair = tuple(sorted((norm1, norm2)))
            if pair in score_cache:
                score = score_cache[pair]
            else:
