...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Optimization: Pre-convert DataFrame to dictionary for O(1) row access
        df1_records = df1.to_dict('index')

        # Optimization: Group row indices by unique normalized values to reduce redundant comparisons
        norm_map1 = {}
        for idx1, row1 in df1_records.items():
            norm1 = normalize_arabic(str(row1[col1]))
            if not norm1 or norm1 == 'nan': continue
            norm_map1.setdefault(norm1, []).append(idx1)
            
        norm_map2 = {}
        # Optimization: Use values directly for faster iteration when building the map
        col2_values = df2[col2].astype(str).values
        for idx2, val2 in enumerate(col2_values):
            norm2 = normalize_arabic(val2)
            if not norm2 or norm2 == 'nan': continue
            norm_map2.setdefault(norm2, []).append(idx2)

        # Optimization: Cache fuzzy matching results between unique normalized strings
        fuzz_results_cache = {}

        for n1, indices1 in norm_map1.items():
            # 1. Handle exact matches efficiently
            if n1 in norm_map2:
                for idx1 in indices1:
                    row1_dict = df1_records[idx1]
                    for idx2 in norm_map2[n1]:
            ...
            # 2. Handle fuzzy matches
            for n2, indices2 in norm_map2.items():
                if n1 == n2: continue
                
                # Check cache for this pair (symmetry allows sorting to halve computations)
                ...
                if score >= 50:
                    ...
                    location_msg = f"Score: {score}%, {col1} vs {col2}"
                    metadata_msg = f"File1: {s1}, File2: {s2}"

                    for idx1 in indices1:
                        row1_dict = df1_records[idx1]
                        for idx2 in indices2:
                            match_row = row1_dict.copy()
                            match_row['Similarity Location'] = location_msg
                            match_row['Source Metadata'] = metadata_msg
