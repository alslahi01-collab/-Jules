...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    df1_norm = df1[col1].astype(str).apply(normalize_arabic)
    df2_norm = df2[col2].astype(str).apply(normalize_arabic)

    # Build hash map for O(1) lookups: normalized_value -> [list of indices]
    norm_map2 = {}
    for idx2, val2 in df2_norm.items():
        if not val2 or val2 == 'nan': continue
        if val2 not in norm_map2:
            norm_map2[val2] = []
        norm_map2[val2].append(idx2)

    # Optimization: cache fuzz results for unique pairs
    fuzz_cache = {}
    unique_norm2 = list(norm_map2.keys())

    for idx1, norm1 in df1_norm.items():
        if not norm1 or norm1 == 'nan': continue
        row1_dict = None # Lazy convert
        ...
        for norm2 in unique_norm2:
            if norm1 == norm2: continue # Handled by 100% matches
