...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Pre-calculate normalized values and group indices to avoid redundant processing
        # and reduce O(N*M) complexity to O(UniqueN * UniqueM)
        norm_map2 = {}
        ...
        # Convert df1 to list of dicts for faster access
        df1_records = df1.to_dict('records')
        df1_indices = df1.index.tolist()

        for i, row_dict in enumerate(df1_records):
            idx1 = df1_indices[i]
            val1 = str(row_dict.get(col1, ''))
            norm1 = normalize_arabic(val1)
            if not norm1 or norm1 == 'nan': continue
            
            # 1. Check for exact matches first (O(1) lookup)
            if norm1 in norm_map2:
                ...
                # If we found an exact match, we still continue to check for fuzzy matches with OTHER values
                # according to the original logic which wasn't exhaustive but compared row-by-row.
                # Actually, the original logic would check ALL idx2 for EACH idx1.

            # 2. Check for fuzzy matches with all UNIQUE values in df2
            for norm2, indices2 in norm_map2.items():
                if norm1 == norm2: continue # already handled in exact match

                # Cache fuzzy results for this pair
