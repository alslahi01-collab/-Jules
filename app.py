...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Pre-convert DataFrames to list of records for faster access
        records1 = df1.to_dict('records')
        records2 = df2.to_dict('records')

        # Group row indices by unique normalized values to reduce comparison complexity from O(N*M) to O(U1*U2)
        norm_map1 = {}
        ...
        for norm1, rows1 in norm_map1.items():
            ...
            for norm2 in unique_norms2:
                ...
                # Cache fuzzy results for unique value pairs
                ...
                if score >= 75:
                    for idx1, row1 in rows1:
                        # Add match for each occurrence in file2
