...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        ...
        # Pre-calculate normalized values and group indices for file 2
        norm_map2 = {}
        ...
        for idx1, row1 in enumerate(records1):
            ...
            # 1. Exact matches (using dictionary lookup for O(1) instead of O(N))
            ...
            # 2. Fuzzy matches (comparing unique values only to reduce expensive fuzz.ratio calls)
            for norm2 in unique_norms2:
                if norm1 == norm2: continue

                # Use a cache for symmetric similarity score
