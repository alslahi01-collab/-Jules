...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    # Use nrows=10 to quickly detect header without loading full sheet
    df1_raw_top = pd.read_excel(xls1, sheet_name=s1, header=None, nrows=10)
    h1 = detect_header_row(df1_raw_top)
    df1 = pd.read_excel(xls1, sheet_name=s1, header=h1)
    df1.columns = df1.columns.astype(str).str.strip()
    
    df2_raw_top = pd.read_excel(xls2, sheet_name=s2, header=None, nrows=10)
    h2 = detect_header_row(df2_raw_top)
    df2 = pd.read_excel(xls2, sheet_name=s2, header=h2)
    df2.columns = df2.columns.astype(str).str.strip()
    
    if col1 in df1.columns and col2 in df2.columns:
        # Pre-convert to records and group by normalized value to reduce redundant fuzzy matching
        # O(N*M) -> O(U1*U2) where U is unique values
        ...
        for n1 in unique_norms1:
            ...
            for n2 in unique_norms2:
                ...
                # Symmetric similarity check using sorted tuple as key
