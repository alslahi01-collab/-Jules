...
def cached_fuzz_ratio(s1, s2):
    """
    Cached version of fuzz.ratio to improve performance in nested loops.
    """
...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Pre-calculate normalized values and exact match lookup for File2 to optimize performance
        df2_processed = []
        norm2_lookup = {}
        for idx2, row2 in df2.iterrows():
            ...
            if norm2 and norm2 != 'nan':
                df2_processed.append((idx2, row2, norm2))
                if norm2 not in norm2_lookup:
                    norm2_lookup[norm2] = []
                norm2_lookup[norm2].append((idx2, row2))
        ...
        for idx1, row1 in df1.iterrows():
            ...
            # ⚡ OPTIMIZATION: Use O(1) dictionary lookup for exact matches
            if norm1 in norm2_lookup:
                for idx2, row2 in norm2_lookup[norm1]:
            ...
            # ⚡ OPTIMIZATION: Use cached fuzzy ratio and pre-normalized values
            for idx2, row2, norm2 in df2_processed:
