...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    # This allows O(1) exact matches and avoids re-normalizing in nested loops
    groups1 = {}
    for idx1, row1 in enumerate(records1):
        norm1 = normalize_arabic(str(row1.get(col1, '')))
        if not norm1 or norm1 == 'nan': continue
        groups1.setdefault(norm1, []).append(idx1)

    groups2 = {}
    for idx2, row2 in enumerate(records2):
        norm2 = normalize_arabic(str(row2.get(col2, '')))
        if not norm2 or norm2 == 'nan': continue
        groups2.setdefault(norm2, []).append(idx2)

    # 1. Exact Matches (100%) - O(N) using hash map
    for norm1, indices1 in groups1.items():
        if norm1 in groups2:
            indices2 = groups2[norm1]
    ...
    # 2. Fuzzy Matches (75-99% and 50-74%)
    # Use unique normalized strings to avoid redundant fuzz.ratio calls
    unique_norms1 = list(groups1.keys())
    unique_norms2 = list(groups2.keys())

    # Cache results of fuzz.ratio for pairs of strings
    fuzzy_cache = {}
    ...
    for norm1 in unique_norms1:
        for norm2 in unique_norms2:
            ...
            # Symmetric cache to avoid duplicate calculations (a,b) and (b,a)
            ...
            if score >= 50:
                indices1 = groups1[norm1]
                indices2 = groups2[norm2]
                for idx1 in indices1:
                    row1 = records1[idx1]
                    for idx2 in indices2:
                        match_row = row1.copy()
