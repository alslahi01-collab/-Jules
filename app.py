...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    # Pre-convert to records to avoid row access overhead in the inner loop
    records1 = df1.to_dict('records')
    records2 = df2.to_dict('records')

    # Map unique normalized values to indices for O(1) matching and reduced fuzzy calls
    norm_map1 = {}
    ...
    for norm1, indices1 in norm_map1.items():
        ...
        # 2. Fuzzy matches (Checked once per unique normalized value pair)
        for norm2, indices2 in norm_map2.items():
            if norm1 == norm2:
                continue # Already handled in exact match logic

            # Use cached result for (norm1, norm2) pair; fuzz.ratio is symmetric
