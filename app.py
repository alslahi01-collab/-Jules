...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Group indices by normalized value to avoid redundant computations
        norm_map1 = {}
        for idx1, val1 in enumerate(df1[col1]):
            n1 = normalize_arabic(str(val1))
            if not n1 or n1 == 'nan': continue
            norm_map1.setdefault(n1, []).append(idx1)
            
        norm_map2 = {}
        for idx2, val2 in enumerate(df2[col2]):
            n2 = normalize_arabic(str(val2))
        ...
        for n1 in unique_norms1:
            ...
            # Pre-convert rows for n1 to dictionaries once
            rows1_dicts = [(i1, df1.iloc[i1].to_dict()) for i1 in indices1]

            for n2 in unique_norms2:
                indices2 = norm_map2[n2]
                
                if n1 == n2:
                    for i1, r1_dict in rows1_dicts:
                        for i2 in indices2:
                ...
                # Use symmetric cache for fuzzy matching
