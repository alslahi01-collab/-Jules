...
def process_comparison(path1, path2, col1, col2):
    ...
    # Pre-cache column names for all sheets in xls2 to avoid redundant I/O
    sheets2_cols = {}
    for s2 in sheets2:
        ...
        sheets2_cols[s2] = set(df2_tmp.columns.astype(str).str.strip())
    ...
    if len(sheets1) == 1 and len(sheets2) == 1:
        ...
        for s1 in sheets1:
            ...
            if not best_s2 or best_score < 100:
                ...
                for s2 in sheets2:
                    df2_cols = sheets2_cols.get(s2, set())
...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        ...
        for idx1, row1 in df1.iterrows():
            ...
            if norm1 not in norm_map1: norm_map1[norm1] = []
            norm_map1[norm1].append((idx1, row1.to_dict()))

        norm_map2 = {}
        for idx2, row2 in df2.iterrows():
            norm2 = normalize_arabic(str(row2[col2]))
            if not norm2 or norm2 == 'nan': continue
            if norm2 not in norm_map2: norm_map2[norm2] = []
            norm_map2[norm2].append((idx2, row2.to_dict()))

        unique_norms2 = list(norm_map2.keys())

        for norm1, rows1 in norm_map1.items():
            # Exact matches (O(1))
            if norm1 in norm_map2:
                for idx1, r1_dict in rows1:
                    for idx2, r2_dict in norm_map2[norm1]:
                        match_row = r1_dict.copy()
            ...
            # Fuzzy matches (only against unique values in df2)
