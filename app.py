...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Pre-process DataFrames into records for faster access
        records1 = df1.to_dict('records')
        records2 = df2.to_dict('records')

        # Group indices by normalized value to reduce redundant fuzzy comparisons
        norm_map1 = {}
        ...
        # Fuzzy matching cache to avoid recomputing scores for same string pairs
        fuzz_results_cache = {}

        for norm1, indices1 in norm_map1.items():
            # 1. Direct O(1) matching for identical values
            ...
            # 2. Fuzzy matching across unique normalized value pairs
            for norm2, indices2 in norm_map2.items():
                if norm1 == norm2: continue # Handled by exact matching above
