...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Optimization: Pre-calculate normalized values and group indices
        # This reduces the complexity from O(N*M) to O(U1*U2) where U is unique values
        ...
        for idx, row in enumerate(records1):
            ...
            if norm and norm != 'nan':
                groups1.setdefault(norm, []).append(idx)

        groups2 = {}
        for idx, row in enumerate(records2):
            norm = normalize_arabic(str(row[col2]))
            if norm and norm != 'nan':
                groups2.setdefault(norm, []).append(idx)

        # Cache for fuzzy match results between unique string pairs
        fuzz_results_cache = {}

        for norm1, indices1 in groups1.items():
            for norm2, indices2 in groups2.items():
                # Exact match
                if norm1 == norm2:
                ...
                # Fuzzy match with symmetric caching
