...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Pre-convert DataFrame 1 to dictionary for faster row access
        # and pre-calculate normalized values to group indices
        ...
        # Optimized comparison using unique value pairs
        for n1, idxs1 in norm_map1.items():
            for n2, idxs2 in norm_map2.items():
                if n1 == n2:
                    for i1 in idxs1:
                        row1 = df1_dicts[i1]
                        for i2 in idxs2:
                ...
                # Cache fuzzy similarity results between unique strings
                # Sorted tuple as key ensures symmetry: (a, b) == (b, a)
