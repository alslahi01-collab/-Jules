...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Optimization: Pre-convert to records and pre-normalize
        records1 = df1.to_dict('records')
        records2 = df2.to_dict('records')

        # Group df2 indices by normalized value for O(1) exact matching
        norm_map2 = {}
        for idx2, rec2 in enumerate(records2):
        ...
        for idx1, rec1 in enumerate(records1):
            ...
            # 2. Fuzzy Match Check (Unique pairs only)
            for norm2, indices2 in norm_map2.items():
                if norm1 == norm2: continue # Handled by exact match
                
                # Cache fuzzy results to avoid redundant calculations
