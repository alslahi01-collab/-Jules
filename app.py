...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    """
    Compares two sheets with optimized unique-value grouping and symmetric caching.
    """
    ...
    # Group indices by normalized value to avoid redundant processing of duplicates
    ...
    # Results cache for fuzz.ratio to avoid redundant O(N^2) computations
    fuzz_results_cache = {}

    for norm1, indices1 in map1.items():
        # Handle exact matches (O(1) lookup)
        if norm1 in map2:
            indices2 = map2[norm1]
        ...
        # Handle fuzzy matches between unique value pairs
        for norm2, indices2 in map2.items():
            if norm1 == norm2:
                continue

            # Use symmetric key for caching (sort tuple to handle (a,b) and (b,a) identically)
