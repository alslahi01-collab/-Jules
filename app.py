...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Optimization: Use record-based iteration and unique value grouping
        ...
        for norm1, indices1 in norm_map1.items():
            # 100% Matches (Exact match in normalized form)
            ...
            for norm2, indices2 in norm_map2.items():
                ...
                if score >= 75:
                    for idx1 in indices1:
                        for _ in indices2: # Maintain original behavior of appending for each match in df2
                            match_row = records1[idx1].copy()
