...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Pre-process df1 and df2 into maps of {normalized_value: list of (idx, row)}
        # This reduces redundant normalizations and enables O(U1 * U2) comparison
        norm_map1 = {}
        for idx1, row1 in df1.iterrows():
            val1 = str(row1[col1])
            norm1 = normalize_arabic(val1)
            if not norm1 or norm1 == 'nan': continue
            if norm1 not in norm_map1:
                norm_map1[norm1] = []
            norm_map1[norm1].append((idx1, row1))
        ...
        # 1. Handle exact matches (100% similarity)
        for norm1 in unique_norms1:
            if norm1 in norm_map2:
                for idx1, row1 in norm_map1[norm1]:
            ...
            # 2. Handle fuzzy matches (similarity < 100%)
            for norm2 in unique_norms2:
                if norm1 == norm2:
                    continue
                
                # Use a cache for unique pairs to avoid O(N*M) redundant computations
                ...
                if score >= 75:
                    for idx1, row1 in norm_map1[norm1]:
                        # Replicate original behavior: add row1 for each matching row in df2
                        for _ in range(len(norm_map2[norm2])):
                            match_row = row1.to_dict()
