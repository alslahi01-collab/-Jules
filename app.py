...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Optimization: use to_dict('records') for faster iteration
        records1 = df1.to_dict('records')
        records2 = df2.to_dict('records')

        # Optimization: Group indices by normalized values
        grouped1 = {}
        for i, rec in enumerate(records1):
            norm = normalize_arabic(str(rec.get(col1, "")))
            if norm and norm != 'nan':
                grouped1.setdefault(norm, []).append(i)
                
        grouped2 = {}
        for j, rec in enumerate(records2):
            norm = normalize_arabic(str(rec.get(col2, "")))
            if norm and norm != 'nan':
                grouped2.setdefault(norm, []).append(j)

        # 1. Exact matches (O(N))
        common_norms = set(grouped1.keys()) & set(grouped2.keys())
        for norm in common_norms:
            for i in grouped1[norm]:
                for j in grouped2[norm]:
                    match_row = records1[i].copy()
                    match_row['Similarity Location'] = f"Row {i+h1+2} in {s1} vs Row {j+h2+2} in {s2}"
                    match_row['Source Metadata'] = f"File1: {s1}, File2: {s2}"
                    matches_100.append(match_row)

        # 2. Fuzzy matches (Optimized: only unique pairs)
        unique_norms1 = list(grouped1.keys())
        unique_norms2 = list(grouped2.keys())

        fuzzy_cache = {}  # Cache for unique (norm1, norm2) pairs
        ...
        for norm1 in unique_norms1:
            for norm2 in unique_norms2:
                ...
                if score >= 75:
                    for i in grouped1[norm1]:
                        for j in grouped2[norm2]:
                            match_row = records1[i].copy()
