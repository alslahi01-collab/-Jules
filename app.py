...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Optimization: Pre-calculate normalized values and group by them
        # Group df2 by normalized values
        norm_map2 = {}
        ...
        # Unique normalized values for fuzzy matching
        unique_norms2 = list(norm_map2.keys())
        # Cache for fuzzy results to avoid re-calculating same pairs
        fuzz_results_cache = {}
        ...
        for idx1, row1 in df1.iterrows():
            ...
            if norm1 in norm_map2:
                ...
                # If we have an exact match, we continue as per original logic's behavior
                # (The original code used 'continue' to skip fuzzy matching if an exact match was found for THAT specific pair)
                # Actually, the original code had:
                # for idx2, row2 in df2.iterrows():
                #    if norm1 == norm2: ...; continue;
                #    score = fuzz.ratio(norm1, norm2)
                # So if norm1 == norm2 for a specific row2, it skipped fuzzy matching with THAT row2.
                # Since we checked ALL exact matches for norm1, we can proceed to fuzzy matching for non-exact values.

            # 2. Fuzzy matches
            for norm2 in unique_norms2:
                if norm1 == norm2: continue # Skip exact matches (already handled)
                ...
                if score >= 75:
                    for idx2, row2 in norm_map2[norm2]:
                        match_row = row1.to_dict()
