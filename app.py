...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Pre-convert to list of dicts to avoid iterrows() overhead
        records1 = df1.to_dict('records')
        records2 = df2.to_dict('records')

        # Group indices by normalized values to avoid redundant fuzzy matching
        norm_map2 = {}
        ...
        for idx1, row1 in enumerate(records1):
            ...
            if norm1 in norm_map2:
                ...
                # If we have an exact match, original logic continued to check other rows for fuzzy matches.
                # However, usually exact matches are sufficient.
                # Re-reading original logic: it checked ALL row pairs.
                # To maintain 100% identical results, we must check other non-identical norm2 values for fuzzy matches.

            # 2. Check fuzzy matches for non-identical values
            for norm2, indices2 in norm_map2.items():
                if norm1 == norm2: continue

                # Use sorted tuple as cache key to leverage symmetry of fuzz.ratio
