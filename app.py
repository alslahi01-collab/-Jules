...
def process_sheets(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        # Pre-calculate normalized values for df2 and group by normalized value
        # This allows O(1) exact match lookup and reduces fuzzy calls to unique pairs
        norm_map2 = {}
        for idx2, row2 in df2.iterrows():
            ...
            if n2 not in norm_map2:
                norm_map2[n2] = []
            norm_map2[n2].append((idx2, row2))

        unique_norms2 = list(norm_map2.keys())
        fuzz_results_cache = {} # Cache scores for unique (norm1, norm2) pairs
        ...
        for idx1, row1 in df1.iterrows():
            ...
            # 2. Fuzzy Match Check (Only against other unique values)
