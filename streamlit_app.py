...
def process_comparison(file1, file2, col1, col2):
    ...
    # Optimization: Pre-read and normalize all sheets of file2 to avoid redundant processing in nested loop
    sheets2_data = []
    for s2 in sheets2:
        ...
        if col2 in df2.columns:
            ...
            for idx2, val2 in enumerate(col2_values):
                n2 = normalize_arabic(val2)
                if not n2 or n2 == 'nan': continue
                norm_map2.setdefault(n2, []).append(idx2)
    ...
    for s1 in sheets1:
        ...
        # Optimization: Pre-convert df1 to dict for O(1) row access
        df1_records = df1.to_dict('index')
        norm_map1 = {}
        for idx1, row1 in df1_records.items():
            n1 = normalize_arabic(str(row1[col1]))
            if not n1 or n1 == 'nan': continue
            norm_map1.setdefault(n1, []).append(idx1)

        for s2_info in sheets2_data:
            s2 = s2_info['name']
            h2 = s2_info['header_idx']
