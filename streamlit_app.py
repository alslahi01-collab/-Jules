...
def process_comparison(file1, file2, col1, col2):
    ...
    # Pre-process target sheets from file2
    processed_sheets2 = []
    for s2 in sheets2:
        ...
        if col2 in df2.columns:
            ...
            for i, rec in enumerate(records2):
                ...
                val_to_indices2.setdefault(norm, []).append(i)

            processed_sheets2.append({
                'name': s2,
                'header_idx': h2,
                'records': records2,
                'val_to_indices': val_to_indices2,
                'unique_norms': list(val_to_indices2.keys())
            })
    ...
    for s1 in sheets1:
        ...
        for i, rec in enumerate(records1):
            ...
            if not norm or norm == 'nan': continue
            val_to_indices1.setdefault(norm, []).append(i)

        unique_norms1 = list(val_to_indices1.keys())

        for ps2 in processed_sheets2:
            s2 = ps2['name']
            h2 = ps2['header_idx']
            records2 = ps2['records']
            val_to_indices2 = ps2['val_to_indices']
            unique_norms2 = ps2['unique_norms']

            for norm1 in unique_norms1:
                indices1 = val_to_indices1[norm1]

                # Exact matches
                if norm1 in val_to_indices2:
                    indices2 = val_to_indices2[norm1]
