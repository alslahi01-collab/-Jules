...
def process_comparison(file1, file2, col1, col2):
    ...
    # Pre-read all sheets of file2 to avoid redundant I/O in the nested loop
    sheets2_data = {}
    for s2 in sheets2:
        ...
        if col2 in df2.columns:
            ...
            # Pre-calculate normalized values and group by unique values
            norm_map2 = {}
            ...
            sheets2_data[s2] = {
                ...
                'norm_map': norm_map2,
                'header_idx': h2,
                'unique_norms': list(norm_map2.keys())
            }
    ...
    for s1 in sheets1:
        ...
        for s2, s2_info in sheets2_data.items():
            records2 = s2_info['records']
            norm_map2 = s2_info['norm_map']
            h2 = s2_info['header_idx']
            unique_norms2 = s2_info['unique_norms']
            fuzz_results_cache = {}
            ...
            for idx1, row1 in enumerate(records1):
                ...
                # 2. Fuzzy matches (Only against unique normalized values in df2)
                for n2 in unique_norms2:
                    if n2 == norm1:
                        continue # Already handled in exact matches
                    # Cache fuzz results for pairs of normalized strings
                    ...
                    if score >= 50:
                        # If match found, add for all rows in df2 that have this normalized value
                        for idx2 in norm_map2[n2]:
                            match_row = row1.copy()
                            match_row['Source Metadata'] = f"File1: {s1}, File2: {s2}"
                            if score >= 75:
