...
def process_comparison(file1, file2, col1, col2):
    ...
    # 1. Pre-load all sheets from file 2 to avoid redundant I/O in nested loops
    target_sheets_data = []
    for s2 in sheets2:
        ...
        if col2 in df2.columns:
            # Group by normalized value for faster matching
            norm_map2 = {}
            for idx2, row2 in df2.iterrows():
                ...
                if n2 not in norm_map2:
                    norm_map2[n2] = []
                norm_map2[n2].append((idx2, row2, h2))
    ...
    # 2. Comparison logic with optimization
    fuzz_results_cache = {} # Cache for unique (norm1, norm2) pairs across all sheets
    ...
    for s1 in sheets1:
        ...
        for idx1, row1 in df1.iterrows():
            ...
            for target in target_sheets_data:
                s2 = target['name']
                norm_map2 = target['norm_map']
                unique_norms2 = target['unique_norms']

                # Exact Match (O(1))
                if norm1 in norm_map2:
                    for idx2, row2, h2 in norm_map2[norm1]:
                ...
                for n2 in unique_norms2:
                    ...
                    if score >= 50:
                        for idx2, row2, h2 in norm_map2[n2]:
                            match_row = row1.to_dict()
                            match_row['Source Metadata'] = f"File1: {s1}, File2: {s2}"
                            if score >= 75:
