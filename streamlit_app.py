...
def process_comparison(file1, file2, col1, col2):
    """
    Compares two Excel files and finds similar rows based on specified columns.
    Optimized with pre-loading sheets, record-based iteration, and unique value grouping.
    """
    ...
    # Pre-process all sheets of the second file to avoid repeated I/O in the nested loop
    target_sheets_data = []
    for s2 in sheets2:
        ...
        if col2 in df2.columns:
            ...
            # Group by unique normalized values
            grouped2 = {}
            for idx2, row2 in enumerate(records2):
                v2 = str(row2.get(col2, ""))
                n2 = normalize_arabic(v2)
                if not n2 or n2 == 'nan': continue
                if n2 not in grouped2:
                    grouped2[n2] = []
                grouped2[n2].append(idx2)

            target_sheets_data.append({
                'name': s2,
                'header_offset': h2,
                'records': records2,
                'grouped': grouped2,
                'unique_norms': list(grouped2.keys())
            })
    ...
    for s1 in sheets1:
        ...
        for target in target_sheets_data:
            s2 = target['name']
            h2 = target['header_offset']
            records2 = target['records']
            grouped2 = target['grouped']
            unique_norms2 = target['unique_norms']

            for idx1, row1 in enumerate(records1):
                v1 = str(row1.get(col1, ""))
                norm1 = normalize_arabic(v1)
