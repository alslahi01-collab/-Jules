...
def process_comparison(file1, file2, col1, col2):
    ...
    # Pre-read all sheets from file2 to avoid redundant reading inside the loop
    sheets2_data = {}
    for s2 in sheets2:
        ...
        if col2 in df2.columns:
            # Pre-calculate normalized values and build index map
            ...
            sheets2_data[s2] = {
                'df_records': df2.to_dict('records'),
                'h2': h2,
                'norm2_map': norm2_map,
    ...
    for s1 in sheets1:
        ...
        for s2, s2_info in sheets2_data.items():
            norm2_map = s2_info['norm2_map']
            unique_norm2 = s2_info['unique_norm2']
            h2 = s2_info['h2']
