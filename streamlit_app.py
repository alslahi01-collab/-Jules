...
def process_comparison(file1, file2, col1, col2):
    ...
    # Cache for fuzzy similarity results between unique normalized string pairs
    fuzz_results_cache = {}

    # Pre-read and pre-process sheets from file2 to avoid redundant processing
    processed_sheets2 = []
    for s2 in sheets2:
        ...
        # Group df2 by normalized value for O(1) exact matching and efficient fuzzy matching
        norm2_map = {}
        for idx2, row2 in df2.iterrows():
            norm2 = normalize_arabic(str(row2[col2]))
            if norm2 and norm2 != 'nan':
                if norm2 not in norm2_map:
                    norm2_map[norm2] = []
                norm2_map[norm2].append((idx2, row2))
        processed_sheets2.append((s2, h2, norm2_map))
    ...
    for s1 in sheets1:
        ...
        # Pre-calculate normalized values for df1
        df1_processed = []
        for idx1, row1 in df1.iterrows():
            norm1 = normalize_arabic(str(row1[col1]))
            if norm1 and norm1 != 'nan':
                df1_processed.append((idx1, row1, norm1))

        for s2, h2, norm2_map in processed_sheets2:
            for idx1, row1, norm1 in df1_processed:
                # 1. Exact matches (100%)
                if norm1 in norm2_map:
                    for idx2, row2 in norm2_map[norm1]:
                ...
                # 2. Fuzzy matches
                for norm2, target_list in norm2_map.items():
                    if norm1 == norm2:
                        continue

                    # Use cache key to avoid redundant fuzzy calculations
                    cache_key = tuple(sorted((norm1, norm2)))
                    if cache_key in fuzz_results_cache:
                        score = fuzz_results_cache[cache_key]
                    else:
                        score = fuzz.ratio(norm1, norm2)
                        fuzz_results_cache[cache_key] = score

                    if score >= 75:
                        for idx2, row2 in target_list:
                            match_row = row1.to_dict()
