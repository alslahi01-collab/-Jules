...
def process_comparison(file1, file2, col1, col2):
    ...
    # Pre-process target file sheets to avoid repeated reading in nested loops
    target_sheets_data = []
    for s2 in sheets2:
        ...
        if col2 in df2.columns:
            ...
            for idx, rec in enumerate(records2):
                norm = normalize_arabic(str(rec[col2]))
                if norm and norm != 'nan':
                    grouped2.setdefault(norm, []).append(idx)
    ...
    # Cache for fuzzy scores across sheets
    fuzzy_cache = {}
    ...
    for s1 in sheets1:
        ...
        for idx, rec in enumerate(records1):
            norm = normalize_arabic(str(rec[col1]))
            if norm and norm != 'nan':
                grouped1.setdefault(norm, []).append(idx)

        for target in target_sheets_data:
            s2 = target['name']
            h2 = target['header_idx']
            ...
            for norm1, indices1 in grouped1.items():
                ...
                # 2. Fuzzy matches
                for norm2, indices2 in grouped2.items():
                    if norm1 == norm2:
                        continue
