...
def process_sheets_orig(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 in df1.columns and col2 in df2.columns:
        for idx1, row1 in df1.iterrows():
            val1 = str(row1[col1])
            norm1 = normalize_arabic.__wrapped__(val1) # Bypass cache for fair comparison if needed, but orig didn't have cache
...
def process_sheets_opt(xls1, s1, xls2, s2, col1, col2, matches_100, matches_75_99, matches_50_74):
    ...
    if col1 not in df1.columns or col2 not in df2.columns: return
...
def generate_test_data(n_rows=300):
    np.random.seed(42)
    data1 = {'Name': [f"Name_{i%50}" for i in range(n_rows)], 'Val': np.random.randint(0, 100, n_rows)}
    data2 = {'Name': [f"Name_{i%50}" if i%3!=0 else f"Name_{i%50}_alt" for i in range(n_rows)], 'Val': np.random.randint(0, 100, n_rows)}
...
def run_bench():
    n = 300
    df1, df2 = generate_test_data(n)
    file1, file2 = "b1.xlsx", "b2.xlsx"
    df1.to_excel(file1, index=False)
    df2.to_excel(file2, index=False)
    xls1, xls2 = pd.ExcelFile(file1), pd.ExcelFile(file2)

    # Warm up cache
    normalize_arabic("test")

    start = time.time()
    m1, m2, m3 = [], [], []
    process_sheets_orig(xls1, 'Sheet1', xls2, 'Sheet1', 'Name', 'Name', m1, m2, m3)
    t_orig = time.time() - start

    start = time.time()
    mo1, mo2, mo3 = [], [], []
    process_sheets_opt(xls1, 'Sheet1', xls2, 'Sheet1', 'Name', 'Name', mo1, mo2, mo3)
    t_opt = time.time() - start

    print(f"Original: {t_orig:.4f}s, Matches: {len(m1)}, {len(m2)}, {len(m3)}")
    print(f"Optimized: {t_opt:.4f}s, Matches: {len(mo1)}, {len(mo2)}, {len(mo3)}")
    print(f"Speedup: {t_orig/t_opt:.2f}x")

    os.remove(file1)
    os.remove(file2)

if __name__ == "__main__": run_bench()
